...
`extend_actions` augments an existing active Extend iLungu Club catalogue card identified by `catalogue_key`. It cannot add or replace catalogue cards. Actions are shown only when that catalogue plugin is active. The first valid action retains the legacy `action_label` and `action_url` fields; all actions are available through the additive `actions` field. Inactive catalogue behaviour, activation behaviour, route families, and `tpw_core_frontend_dashboard_plugin_active_url` remain unchanged.
